...
layui.use(['table','form', 'layedit', 'laydate'], function(){
	...
	table.render({
		elem: '#test'
	    ,url: $.PROJECT_URL + "area/findByPagesf.do"
	    ,cols: [[
	    	{field:'addressProvinceName', title: '所属省份'}
	    	,{field:'cscount', title: '开通城市'}
	    	,{field:'qxcount', title: '开通区县'}
	    	,{field:'schoolcount', title: '开通校区'}
	        ...
	        }},{field:'enddate', title: '状态', templet: function(d){
	    	...
	    	,{title: '操作', fixed: 'right', align:'center',width:'220' , toolbar: '#barDemo'}
	    ]]
		,page: true
	});
	...
	table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === 'detail'){ //查看
			layer.open({
				  ...
				  shade: 0.8,
				  area: ['43%', '450px'],
				  content: $.PROJECT_URL +'html/area/updateTime.html?id='+data.id,
				  end:function(){
					  queryreload(table);
				  }
			});
			...
			layer.open({
				  ...
				  shade: 0.8,
				  area: ['43%', '50%'],
				  content: $.PROJECT_URL +'html/area/update.html?id='+data.id,
				  end:function(){
					  queryreload(table);
				  }
			}); 
			...
			layer.open({
				  ...
				  shade: 0.8,
				  area: ['50%', '72%'],
				  content: $.PROJECT_URL +'html/user/list.html?type=5&relationid='+data.id,
				  end:function(){
					  //queryreload(table);
				  }
			}); 
			...
			layer.open({
				  ...
				  shade: 0.8,
				  area: ['43%', '50%'],
				  content: $.PROJECT_URL +'html/user/update.html?type=5&relationid='+data.id,
				  end:function(){
