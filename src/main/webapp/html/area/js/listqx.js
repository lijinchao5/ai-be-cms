...
layui.use(['table','form', 'layedit', 'laydate'], function(){
	...
	table.render({
		elem: '#test'
	    ,url: $.PROJECT_URL + "area/findByPageqx.do"
	    ,cols: [[
	...
	table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		...
		if(layEvent === 'detail'){ //查看
			...
			layer.open({
				  ...
				  shade: 0.8,
				  area: ['50%', '72%'],
				  content: $.PROJECT_URL +'html/user/list.html?type=7&relationid='+data.id,
				  end:function(){
					  //queryreload(table);
				  }
			}); 
			...
			layer.open({
				  ...
				  shade: 0.8,
				  area: ['43%', '50%'],
				  content: $.PROJECT_URL +'html/user/update.html?type=7&relationid='+data.id,
				  end:function(){
