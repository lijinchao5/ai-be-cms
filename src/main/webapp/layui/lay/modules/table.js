      ...
      $.ajax({
        ...
        ,success: function(res){
          if(res[response.statusName] != response.statusCode){
            ...
            //登陆失效验证
            if(res[response.statusName] == $.CONSTANT.NO_LOGIN_CODE){
				var _layer = layer;
				if(window.parent && window.parent.layer){
					_layer = window.parent.layer;
				}
				_layer.alert('登陆状态失效，请重新登陆。', {
					closeBtn: 0
				}, function(){
        	...
        	//TODO 自定义分页数据
        	res[response.countName] = res.result.total;
        	res[response.dataName] = res.result.records;
